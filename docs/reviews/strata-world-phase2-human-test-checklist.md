...
- [ ] Confirming clears terrain and regenerates using the current biome parameters
      NOTE (code fix applied): `resetWorld()` now uses the SERVER_STOPPED pattern — `server.stop(false)` from the server thread returns immediately; the server completes its tick loop, fires SERVER_STOPPED (which deletes all `.mca` files), then closes the network connection; the client detects the connection loss naturally. Fresh chunks regenerate via `BiomeGenerationMixin` when the player reopens the world. The player lands on the world-select screen after confirm. Needs re-test.
...
- [ ] Features and spawns from the draft appear in newly explored chunks on world load
      NOTE (code fix applied): `StrataWorld.SERVER_STARTED` now eagerly initializes `editorPreviewBiome` from the biome registry and loads `dynamicGenerationSettings`/`dynamicSpawnSettings` from `_session.draft.json` before any player joins. Features will appear in any chunk generated AFTER the player joins. Known limitation: spawn chunks (~80 blocks of world spawn) are generated by `prepareStartRegion()` BEFORE `SERVER_STARTED`, so they will not have custom features until the player uses Reset World and walks 200+ blocks away from spawn. Needs re-test.
- [ ] Spawn chunks regenerate correctly after Reset World
      NOTE (resolved by SERVER_STOPPED pattern + SERVER_STARTED eager init): Reset World deletes all `.mca` files after the server fully stops; SERVER_STARTED re-initializes dynamic settings before spawn chunks are generated on next world open. Needs re-test.
